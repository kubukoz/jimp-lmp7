...
skorowidz *utworz_skorowidz(int rozmiar, char **slowa) {
    int i;
    skorowidz *sk = malloc(sizeof (skorowidz));
    sk->slowa = slowa;
...
void dodaj_wystapienia(skorowidz *sk, int nr_slowa, char *linia, int *indeksy, int ilosc_indeksow, int nr_linii) {
    dodaj_linie(&(sk->wystapienia)[nr_slowa], linia, nr_linii, indeksy, ilosc_indeksow);
}
...
void wypisz_skorowidz(skorowidz *sk) {
    ...
    for (i = 0; i < sk->ilosc_slow; i++) {
        ...
        for (j = 0; j < wystapienia_slowa.size; j++) {
            ...
            wypisz_linie(linia.linia, linia.nr, linia.indeksy, linia.ilosc_indeksow, slowo);
        }
